...
% data based on Silder2007 (IDENTIFICATION OF PASSIVE ELASTIC JOINT MOMENT-ANGLE
% RELATIONSHIPS IN THE LOWER EXTREMITY, )
...
% phases in the optimization problem have always the samen parameters
...
% additional constraint that muscle groups have the same values
% (since we for example cannot descriminate between parameters of the vastus
% lateral, medialis and intermdius we impose that the parameters should be the same)
% I hard coded this for now: feel free to improve this :)

% ankle
iPeroneus = [8 9];
opti.subject_to(TR(1).kpe(iPeroneus(1)) == TR(1).kpe(iPeroneus(2)));
opti.subject_to(TR(1).ksf(iPeroneus(1)) == TR(1).ksf(iPeroneus(2)));
disp(['Constraint on muscles ' DatStore(1).MuscleNames(iPeroneus)])
iFlex = [5 6];
opti.subject_to(TR(1).kpe(iFlex(1)) == TR(1).kpe(iFlex(2)));
opti.subject_to(TR(1).ksf(iFlex(1)) == TR(1).ksf(iFlex(2)));
disp(['Constraint on muscles ' DatStore(1).MuscleNames(iFlex)]);
iGas = [1 2];
opti.subject_to(TR(1).kpe(iGas(1)) == TR(1).kpe(iGas(2)));
opti.subject_to(TR(1).ksf(iGas(1)) == TR(1).ksf(iGas(2)));
disp(['Constraint on muscles ' DatStore(1).MuscleNames(iGas)]);

% hip
iGlutMax = [19 20 21];
opti.subject_to(TR(2).kpe(iGlutMax(1)) == TR(2).kpe(iGlutMax(2)));
opti.subject_to(TR(2).kpe(iGlutMax(1)) == TR(2).kpe(iGlutMax(3)));
opti.subject_to(TR(2).ksf(iGlutMax(1)) == TR(2).ksf(iGlutMax(2)));
opti.subject_to(TR(2).ksf(iGlutMax(1)) == TR(2).ksf(iGlutMax(3)));
disp(['Constraint on muscles ' DatStore(2).MuscleNames(iGlutMax)]);

iAddMag = [13 14 15];
opti.subject_to(TR(2).kpe(iAddMag(1)) == TR(2).kpe(iAddMag(2)));
opti.subject_to(TR(2).kpe(iAddMag(1)) == TR(2).kpe(iAddMag(3)));
opti.subject_to(TR(2).ksf(iAddMag(1)) == TR(2).ksf(iAddMag(2)));
opti.subject_to(TR(2).ksf(iAddMag(1)) == TR(2).ksf(iAddMag(3)));
disp(['Constraint on muscles ' DatStore(2).MuscleNames(iAddMag)]);

% knee
iVas = [9 10 11];
opti.subject_to(TR(3).kpe(iVas(1)) == TR(3).kpe(iVas(2)));
opti.subject_to(TR(3).kpe(iVas(1)) == TR(3).kpe(iVas(3)));
opti.subject_to(TR(3).ksf(iVas(1)) == TR(3).ksf(iVas(2)));
opti.subject_to(TR(3).ksf(iVas(1)) == TR(3).ksf(iVas(3)));
disp(['Constraint on muscles ' DatStore(3).MuscleNames(iVas)]);

iKneeFlexBi = [1 2 3];
opti.subject_to(TR(3).kpe(iKneeFlexBi(1)) == TR(3).kpe(iKneeFlexBi(2)));
opti.subject_to(TR(3).kpe(iKneeFlexBi(1)) == TR(3).kpe(iKneeFlexBi(3)));
opti.subject_to(TR(3).kpe(iKneeFlexBi(1)) == TR(3).ksf(iKneeFlexBi(2)));
opti.subject_to(TR(3).kpe(iKneeFlexBi(1)) == TR(3).ksf(iKneeFlexBi(3)));
disp(['Constraint on muscles ' DatStore(3).MuscleNames(iKneeFlexBi)]);
...
legend('estimated','defaultsim','Silder2007');
